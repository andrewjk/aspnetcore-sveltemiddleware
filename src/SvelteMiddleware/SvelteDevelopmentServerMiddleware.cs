...
namespace SvelteMiddleware
{
    internal static class SvelteMiddleware
    {
            ...
            string scriptName, int port = 0, ScriptRunnerType runner = ScriptRunnerType.Npm, string regex = DefaultRegex)
        {
            ...
            // Start Svelte and attach to middleware pipeline
            var appBuilder = spaBuilder.ApplicationBuilder;
            var logger = LoggerFinder.GetOrCreateLogger(appBuilder, LogCategoryName);
            var portTask = StartSvelteServerAsync(sourcePath, scriptName, logger, port, runner, regex);

            // Everything we proxy is hardcoded to target http://localhost because:
            // - the requests are always from the local machine (we're not accepting remote
            //   requests that go directly to the Svelte server)
            // - given that, there's no reason to use https, and we couldn't even if we
            //   wanted to, because in general the Svelte server has no certificate
            ...
            SpaProxyingExtensions.UseProxyToSpaDevelopmentServer(spaBuilder, () =>
            {
                ...
                return targetUriTask.WithTimeout(timeout,
                    $"The Svelte server did not start listening for requests " +
